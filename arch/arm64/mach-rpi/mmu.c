        ...
        unsigned long flags)
{
    ...
    if (pmd_none(pmd)) {
        unsigned long pte_phys = alloc_pgtable();
        ...
        unsigned long flags)
{
    ...
    if (pgd_none(pgd)) {
        unsigned long pud_phys = alloc_pgtable();
...
static void create_identity_mapping(void)
{
    ...
    /* mapping text. */
    start = (unsigned long) _text_boot_start;
    end = (unsigned long) _text_end;
    __create_pgd_mapping((pgd_t *)idmap_pgd_dir, start, start,
                         end - start, PAGE_KERNEL_ROX,
                         early_pgtable_alloc,
                         0);

    /* mapping memory. */
    start = PAGE_ALIGN((unsigned long)_text_end);
    end = TOTAL_MEMORY;  // 512M
    __create_pgd_mapping((pgd_t *)idmap_pgd_dir, start, start,
...
{
    __create_pgd_mapping((pgd_t *)idmap_pgd_dir, PBASE, PBASE,
...
static int enable_mmu(void)
{
    ...
    write_sysreg(idmap_pgd_dir, ttbr0_el1);
    isb();

    write_sysreg(SCTLR_ELx_M, sctlr_el1);
    isb();
    asm("ic iallu");  /* set all cache invalid. */
    dsb(nsh);
    isb();
...
{
    memset(idmap_pgd_dir, 0, PAGE_SIZE);
